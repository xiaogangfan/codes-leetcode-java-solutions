    ...
    public int minDistance(String word1, String word2) {
        ...
        for (int i = 1; i <= word1.length(); i++) {
            for (int j = 1; j <= word2.length(); j++) {
                ...
                distance[i][j] = Math.min(distance[i - 1][j], distance[i][j - 1]) + 1; // 如果char不一样就不用变换了
                distance[i][j] = Math.min(distance[i][j], distance[i - 1][j - 1] + (word1.charAt(i - 1) == word2.charAt(j - 1) ? 0 : 1));
